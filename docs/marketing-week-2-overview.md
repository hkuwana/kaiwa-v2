# Marketing Week 2 Overview: Days 9-14

**Theme**: Community Depth & Second Practice Lab
**Goal**: Build deeper relationships, gather more social proof, refine your approach

---

## Day 9: Video Testimonials & TikTok Series

**Morning** (60 min):
- Respond to all case file comments/shares
- Reach out to 3 Practice Lab participants for video testimonials

**Afternoon** (90 min):
- Create TikTok series: "Things nobody tells you about meeting partner's parents"
  - Video 1: "The gift matters more than you think"
  - Video 2: "They're testing your sincerity, not your grammar"
  - Video 3: "What to do when you don't understand"

**Evening** (30 min):
- Post on 3-5 Reddit threads
- Update tracker with video performance

**Copy-Paste Video Script** (Video 1):
```
Things nobody tells you about meeting your Japanese partner's parents #1:

The gift (お土産) matters MORE than your Japanese level.

Here's why:
• It shows you understand reciprocity (important in Japanese culture)
• It gives them something to talk about (takes pressure off you)
• It shows you prepared and took this seriously

What to bring:
✅ Something from YOUR home city/country
✅ Something they can share (cookies, tea, local snack)
✅ Wrap it nicely (presentation matters)

❌ Don't bring: alcohol (unless you know they drink), something super expensive (creates obligation), nothing at all

The gift is your opening move. Get it right.

Want the full preparation guide? Link in bio.
#bilingualcouple #japanese #meetingtheparents
```

---

## Day 10: LinkedIn Thought Leadership

**Morning** (60 min):
- Engage with 10 other founders' content
- Respond to all comments from yesterday

**Afternoon** (90 min):
- Write LinkedIn thought piece: "Why I'm Building Kaiwa Instead of Another Duolingo Clone"

**Article Outline**:
```
1. The Problem: The gap between "knowing words" and "saying what matters"
2. Why Gamification Fails: Real conversations aren't games
3. The Insight: People need practice for specific moments, not general fluency
4. What I'm Building: A rehearsal space for high-stakes conversations
5. Why This Matters: Language is about connection, not points
6. Building in Public: What I'm learning (invite feedback)
```

**Evening** (30 min):
- Share article to Twitter, email list
- DM article to 10 warm leads

---

## Day 11: Reddit Deep Engagement Day

**Morning** (60 min):
- Find 10 threads related to bilingual relationships
- Post thoughtful 200-300 word comments on each

**Afternoon** (90 min):
- Create "Ask Me Anything" style post:

**Reddit AMA Post Template**:
```
Title: I've helped 20+ people prepare to meet their partner's parents in another language - AMA

Body:
Hi r/LearnJapanese,

I've spent the last month working with people preparing for one of the scariest language moments: meeting your partner's family for the first time.

I've noticed patterns in what people are afraid of, what actually helps, and what makes the difference between awkward silence and genuine connection.

Some context:
• I built this because I couldn't talk to my own Japanese grandparents
• I run free practice sessions every Saturday
• I'm building Kaiwa to help people practice high-stakes conversations

Ask me anything about:
• How to prepare for the meeting
• What parents are actually looking for
• How to handle when you don't understand
• Cultural tips that matter
• How to practice effectively

I'll answer every question!
```

**Evening** (30 min):
- Respond to EVERY comment on your AMA
- Document common questions for future content

---

## Day 12: User Research & Product Insight

**Morning** (60 min):
- Schedule 3 one-on-one calls with users for next week
- Send research interview questions in advance

**Research Questions to Ask**:
1. Walk me through your situation - what conversation are you preparing for?
2. What have you tried so far? What worked? What didn't?
3. On a scale of 1-10, how confident do you feel right now?
4. What would make you feel 100% prepared?
5. If Kaiwa could do ONE thing for you, what would it be?
6. How much would you pay for something that solved this completely?

**Afternoon** (90 min):
- Create detailed comparison content: "Kaiwa vs Duolingo vs Human Tutor vs Language Exchange"

**Comparison Framework**:
```
| Feature | Duolingo | Human Tutor | Language Exchange | Kaiwa |
|---------|----------|-------------|-------------------|-------|
| Cost | Free | $25-50/hr | Free | $19/mo |
| Specific scenarios | ❌ | ✅ (if you find the right tutor) | ❌ | ✅ |
| Cultural context | ❌ | ✅ | ❌ | ✅ |
| 24/7 availability | ✅ | ❌ | ❌ | ✅ |
| Judgment-free | ✅ | 50/50 | ❌ | ✅ |
| Practice high-stakes conversations | ❌ | ✅ | ❌ | ✅ |
```

**Evening** (30 min):
- Share comparison post on LinkedIn, Reddit, Twitter
- Engage with responses

---

## Day 13: Practice Lab Prep & Announcement

**Morning** (60 min):
- Review last week's Practice Lab notes
- Update session materials based on feedback
- Prepare 2-3 new scenarios based on user requests

**Afternoon** (90 min):
- Announce Practice Lab #2 across all platforms

**Announcement Template** (LinkedIn):
```
Practice Lab #2 this Saturday.

Last week: 5 people, 45 minutes, zero judgment.

Result: "This 45-minute session was more valuable than 2 years of Duolingo for this specific conversation." - Attendee

This week's focus:
→ Meeting partner's parents (still the #1 request)
→ NEW: Handling difficult questions ("When will you get married?", "Will you move to Japan?")
→ NEW: Recovery tactics when you freeze mid-conversation

What we'll do:
• Practice the first 5 minutes together
• Roleplay common scenarios
• Personalized Q&A for your situation
• Cultural insights that actually matter

5 spots. Free. No pitch.

Comment "SATURDAY" or DM me if you want in.

Building Kaiwa in public, Day 13.

#buildinginpublic #languagelearning
```

**Evening** (30 min):
- Send personalized invites to 10 warm leads
- Email last week's participants: "Want to come back for round 2?"

---

## Day 14: Practice Lab #2 + Weekly Review

**Morning** (90 min):
- Final prep for Practice Lab #2
- Send materials to registered participants
- Review what you learned from last session

**Afternoon** (60-90 min):
- Host Practice Lab #2
- Take detailed notes during session
- Ask for feedback at the end

**Evening** (90 min):
- Send follow-up email to participants
- Request testimonials
- Document insights

**Weekly Review** (30 min):
- Review Week 2 metrics:
  - How many people engaged with case file?
  - Video performance vs Week 1?
  - Email list growth?
  - How many warm leads converted?
  - Practice Lab #2 vs #1 attendance?
  - Any testimonials collected?
  - Best performing platform?

**Weekly Reflection Questions**:
1. What content format is working best?
2. What platform drives the most engagement?
3. What questions are people asking repeatedly?
4. What surprised you this week?
5. What would you do differently next week?

---

## Week 2 Success Metrics

By end of Week 2, you should have:
- [ ] 10+ warm leads in tracker
- [ ] 2 Practice Labs completed
- [ ] 1-2 testimonials collected
- [ ] First case file published and shared
- [ ] 5+ video pieces created
- [ ] 1 thought leadership article published
- [ ] Email list growing (even if small)
- [ ] Clear picture of what content resonates

---

## Key Copy-Paste Templates for Week 2

### For Practice Lab Follow-Up:
```
Hi [Name],

Thank you for making Practice Lab #2 great!

Quick question: Would you be open to a quick 15-min call this week?

I'd love to hear:
• How the practice helped (or didn't)
• What you're still worried about
• What would make Kaiwa more valuable for you

Your feedback literally shapes what I build.

Available times: [list 3-4 options]

Let me know!
```

### For Video Comments:
```
Yes! This is exactly what I'm trying to solve with Kaiwa.

What's YOUR situation? What conversation are you preparing for?

I run free practice sessions on Saturdays if you want to rehearse it before the real thing.

DM me if you're interested!
```

### For "How much does this cost?" Questions:
```
Right now I'm offering free practice sessions while I build Kaiwa (in exchange for feedback).

Later pricing will likely be $19/month with a commitment model (practice 8x/month, document 1 real conversation, get money back).

But for now: free if you help me understand what you need.

Want to try a session?
```

---

**Next Week**: Week 3 focuses on scaling, creating more case files, and turning testimonials into compelling social proof that drives signups.
