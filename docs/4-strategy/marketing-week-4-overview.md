# Marketing Week 4 Overview: Days 22-28

**Theme**: Conversion & Scaling
**Goal**: Turn warm leads into paying customers, scale what's working, prepare for sustainable operations

---

## Day 22: Beta Launch Announcement

**Morning** (60 min):

- Review all metrics from Weeks 1-3
- Identify your top 10 most engaged users
- Draft beta pricing offer

**Beta Pricing Offer** (Copy & Customize):

```
Kaiwa Beta Launch: Founding Member Offer

After 3 weeks of Practice Labs and working with 20+ people preparing for high-stakes conversations, I'm ready to launch Kaiwa Beta.

**What you get**:
• Unlimited practice scenarios (Meeting parents, apologies, difficult conversations, etc.)
• Weekly group Practice Labs (with feedback)
• 1 personalized 1-on-1 session per month
• Private WhatsApp group with direct founder access
• Lifetime founding member pricing (price locked forever)

**Founding Member Pricing**: $19/month

**The deal**:
I'm looking for 10 founding members who will:
• Use Kaiwa at least 2x per week
• Give me honest feedback monthly
• Share your wins (anonymously if you prefer)
• Help shape what Kaiwa becomes

**Why now**: You've been following the journey. You've seen it built in public. You know it works. This is your chance to lock in founding member pricing before it goes to $29/month.

**Limited to 10 spots** (I need to keep it small to give everyone attention).

Interested? DM me "FOUNDING" and let's talk.
```

**Afternoon** (90 min):

- Post beta announcement on LinkedIn
- Email your list with beta offer
- DM your top 10 engaged users personally

**Personal DM Template**:

```
Hey [Name],

You've been following the Kaiwa journey from [Week X]. Your [engagement/feedback/attendance] has been invaluable.

I'm launching the beta this week with 10 founding member spots.

Given how engaged you've been, I wanted to reach out personally before announcing publicly.

Founding member details:
• $19/month (locked forever)
• Unlimited practice
• Weekly Practice Labs
• 1 personal session per month
• Private WhatsApp group with direct access to me

In exchange: 2x/week usage + monthly feedback.

Would you be interested?

If yes, let's set up a quick call to discuss your specific needs and how Kaiwa can help.

If not, no worries at all - you've already been super helpful!

Either way, thanks for being part of this journey.
```

**Evening** (30 min):

- Respond to all beta inquiries
- Schedule intro calls with interested people

---

## Day 23: Beta Onboarding Calls

**Morning** (60 min):

- Prepare onboarding call script
- Set up payment links (Stripe, Gumroad, or manual PayPal)
- Create simple onboarding doc

**Onboarding Call Script** (15 min per call):

```
**Agenda**:
1. Thank them for being an early believer
2. Understand their specific situation:
   - What conversation are they preparing for?
   - Timeline?
   - Biggest worry?
   - What would success look like?
3. Walk through how Kaiwa works
4. Set expectations:
   - What's ready now
   - What's coming soon
   - How to give feedback
5. Answer questions
6. Process payment (if they're ready)
7. Schedule their first 1-on-1 session

**Key questions to ask**:
- "What made you decide to join as a founding member?"
- "What's the one feature that would make this a no-brainer?"
- "How much would you be willing to pay if this completely solved your problem?"
```

**Afternoon** (90 min):

- Conduct 3-5 onboarding calls
- Take detailed notes on each
- Follow up with payment links

**Evening** (30 min):

- Send welcome email to new members
- **Add them to WhatsApp group** (recommended - see detailed strategy below)
- Update tracker with payment status

### WhatsApp Group Setup (Recommended)

> **Full strategy**: See [whatsapp-founding-member-strategy.md](./whatsapp-founding-member-strategy.md)

**Why WhatsApp > Discord/Slack:**

- ✅ Zero friction (everyone already has it)
- ✅ International-friendly (your users are global)
- ✅ High engagement (people check it constantly)
- ✅ Personal feel (not corporate)

**How to set up:**

1. **Create group**: `Kaiwa Founding Members 🗣️`
2. **Get their WhatsApp number**: In the onboarding call or welcome email
3. **Send personal DM first** (before adding to group):

   ```
   Hey [Name]! Welcome to Kaiwa as a founding member 🎉

   Quick logistics:
   • I'm adding you to our private WhatsApp group (10 founding members only)
   • We use it for quick feedback, sharing wins, and supporting each other
   • I'm active in there daily - ask me anything

   What's your WhatsApp number?

   Also - when is your conversation happening? Want to make sure we're prepped!
   ```

4. **Add to group with intro**:

   ```
   Everyone, meet [Name]! 👋

   [Name] is preparing for [their specific situation]

   [Name], welcome! This group is your direct line to me + support from other founding members.
   Feel free to ask questions, share practice updates, or just lurk - whatever helps!
   ```

5. **Daily WhatsApp rhythm** (10-15 min/day):
   - Morning: Check messages, respond, post motivational check-in
   - Evening: Celebrate wins, answer questions

**Welcome Email Template**:

```
Subject: Welcome to Kaiwa! Here's how to get started

Hi [Name]!

Welcome to Kaiwa as a founding member! 🎉

You're one of 10 people shaping what this becomes. Thank you for believing in this early.

**Here's what happens now**:

1. **Join the WhatsApp group**: What's your WhatsApp number? I'll add you to our private founding members group (direct access to me + other members)
2. **Your first practice session**: [Link to schedule or instructions]
3. **This week's Practice Lab**: Saturday at [time] - [Zoom link]
4. **Your 1-on-1 session**: Reply to this email with 3 times that work for you

**How to get the most out of Kaiwa**:
• Practice 2x per week minimum (even just 10 minutes)
• Come to weekly Practice Labs
• Join the WhatsApp group (daily support + community)
• Share your wins in the group (they inspire others!)
• Tell me what's broken (I fix fast)

**Your feedback matters**:
I'll check in with you monthly. But anytime you have thoughts, just reply to this email or drop it in the WhatsApp group.

Questions? I'm here (email or WhatsApp once you're added).

Let's help you have that conversation you're preparing for.

[Your name]

P.S. What's your timeline? When is your actual conversation happening?
```

---

## Day 24: Content Scaling Day

**Morning** (60 min):

- Analyze best performing content from Weeks 1-3
- Identify top 3 content types by:
  - Engagement
  - Time to create
  - Conversion to signups

**Afternoon** (90 min):

- Create content templates for your top-performing formats

**Example Template Library**:

**TikTok Script Template**:

```
Hook (2 seconds): [State the problem/fear]

Context (5 seconds): [Who this is for]

Value (30 seconds):
• Point 1: [Specific advice]
• Point 2: [Specific advice]
• Point 3: [Specific advice]

CTA (3 seconds): [What to do next]

Hashtags: #languagelearning #[targetlanguage] #[situation] #relationshipgoals
```

**Case File Template**:

```
Headline: How [Name] [accomplished the goal]

Section 1: The Situation (100 words)
- Who they are
- What conversation they needed to have
- Timeline/pressure

Section 2: The Fear (100 words)
- What they were really afraid of (not grammar)
- How it was affecting them

Section 3: The Preparation (200 words)
- What we focused on
- Specific phrases practiced
- The approach/framework

Section 4: The Breakthrough (100 words)
- The moment it clicked
- What changed

Section 5: The Result (100 words)
- How the real conversation went
- What happened after
- Testimonial quote

Section 6: If You're in [Their] Shoes (100 words)
- Application to reader
- CTA to Practice Lab or Kaiwa

Word count: 600-700 words
```

**Reddit Value Comment Template**:

```
[Acknowledge their situation specifically]

I've worked with several people in your exact situation. Here's what consistently helps:

**[Area 1]**: [Specific tactical advice]

**[Area 2]**: [Specific tactical advice]

**[Area 3]**: [Specific tactical advice]

The key insight most people miss: [Deeper psychological or cultural insight]

**Your gameplan**:
1. [First action step]
2. [Second action step]
3. [Third action step]

[Soft CTA: "Happy to help if you want to talk through your specific situation" or "I run practice sessions on Saturdays if you want to rehearse this"]

Good luck - you've got this!
```

**Evening** (30 min):

- Create 5 pieces of content using templates
- Schedule posts for rest of week

---

## Day 25: Testimonial Campaign

**Morning** (60 min):

- Compile ALL testimonials collected (Weeks 1-4)
- Organize by:
  - Text vs video
  - Persona type (Sofia, David, etc.)
  - Outcome type (confidence, success, breakthrough)

**Afternoon** (90 min):

- Create testimonial showcase content

**LinkedIn Testimonial Post**:

```
3 weeks ago, I hosted my first Practice Lab.

Today, I have 10 founding members.

Here's what they're saying:

---

"This 45-minute session was more valuable than 2 years of Duolingo for this specific conversation."
- Sarah, 29

---

"I've been so afraid of sounding stupid that I chose silence. Silence is worse. This helped me try."
- David, 35

---

"I just FELT the words instead of thinking about them. That's when I knew I could do this."
- Maria, 28

---

This is why I'm building Kaiwa.

Not for points or streaks.
For the moments that actually matter.

The conversations you're afraid of are the ones worth practicing.

Building in public, Day 25.

[If you're preparing for a high-stakes conversation, DM me "PREPARE" and let's talk.]

#buildinginpublic #testimonials #kaiwa
```

**Instagram Carousel** (10 slides):

- Slide 1: "What People Are Saying About Kaiwa"
- Slides 2-9: Individual testimonials (one per slide)
- Slide 10: "Join Practice Lab this Saturday" + CTA

**Evening** (30 min):

- Share testimonial content across all platforms
- Engage with every comment

---

## Day 26: Referral System Launch

**Morning** (60 min):

- Design simple referral offer

**Referral Offer**:

```
Founding Member Referral Program

For every friend you refer who becomes a founding member:
• You get: 1 extra 1-on-1 session (worth $49)
• They get: Founding member pricing + priority onboarding

How it works:
1. Share your unique referral link (I'll send you one)
2. When they sign up and mention you, you both get rewards
3. No limit on how many you can refer

Why this matters:
The best way to prepare for a high-stakes conversation? Practice with someone else preparing for the same thing.

Let's build this community together.
```

**Afternoon** (90 min):

- Email referral offer to founding members
- Create referral tracking spreadsheet
- Make it easy: Give each member a unique code or link

**Evening** (30 min):

- Post about referral program on social
- Encourage founding members to share their experience

---

## Day 27: Prep for Sustainable Rhythm

**Morning** (60 min):

- Review 4 weeks of data
- Document what's working, what's not

**Afternoon** (90 min):

- Create your sustainable weekly schedule

**Sustainable Marketing Schedule** (Post-Week 4):

**Monday** (90 min):

- Respond to all weekend engagement
- Post 1 Reddit value comment
- Plan week's content

**Tuesday** (90 min):

- Create & post 1 case file OR thought leadership article
- Engage with 10 other creators' content

**Wednesday** (60 min):

- Film/create 3-5 videos for the week
- Engage with Reddit threads (3-5 comments)

**Thursday** (60 min):

- Email newsletter to list
- Personal outreach to 5 warm leads
- Post video content

**Friday** (60 min):

- Practice Lab prep
- Send materials to participants
- Final outreach for weekend session

**Saturday** (2 hours):

- Host Practice Lab
- Gather feedback and testimonials

**Sunday** (60 min):

- Weekly review and planning
- Document insights
- Prep next week's content

**Evening** (30 min):

- Write up sustainability plan
- Share plan with founding members (get feedback)

---

## Day 28: Practice Lab #4 + Month 1 Reflection

**Morning** (90 min):

- Final prep for Practice Lab #4
- Include founding members in separate "advanced" session (test tiered model)

**Afternoon** (90 min):

- Host Practice Lab #4 (maybe split into two sessions: general + founding members)

**Evening** (2-3 hours):

- Month 1 Complete Reflection

**Month 1 Metrics to Review**:

**Awareness**:

- Total content pieces created
- Total views/impressions across platforms
- Platform breakdown (which performed best)
- Engagement rate trends

**Acquisition**:

- Email list size (starting vs now)
- Total warm leads in tracker
- Reddit karma gained
- LinkedIn followers/connections added
- TikTok/Instagram followers

**Engagement**:

- Practice Labs attended (total headcount across 4 weeks)
- Repeat attendees (how many came 2+ times)
- Email open rates
- Comment/reply rates

**Conversion**:

- Founding members signed up
- Revenue generated (if any)
- Average time from first touch to signup
- Conversion rate (warm leads → customers)

**Product**:

- Common feature requests
- Most requested scenarios
- Biggest pain points users mentioned
- What would make them pay more

**Reflection Questions**:

1. **What worked better than expected?**
2. **What didn't work at all?**
3. **What content should you 2x?**
4. **What should you stop doing?**
5. **What surprised you most?**
6. **What did you learn about your users?**
7. **What did you learn about yourself?**
8. **What would you change about Week 1 if you could redo it?**
9. **Are you closer to product-market fit? How do you know?**
10. **What's the ONE thing to focus on in Month 2?**

---

## Week 4 Success Metrics

By end of Week 4, you should have:

- [ ] 5-10 founding members (paying customers)
- [ ] $95-190 MRR (from founding members)
- [ ] 30+ warm leads in tracker
- [ ] 4 Practice Labs completed
- [ ] 8+ testimonials collected (text or video)
- [ ] 3+ case files published
- [ ] 25+ video pieces created
- [ ] Email list of 50-100 people
- [ ] Sustainable weekly rhythm documented
- [ ] Clear product roadmap based on user feedback
- [ ] Referral system live

---

## Preparing for Month 2

**What to focus on next month**:

1. **Retention**: Keep founding members engaged
2. **Referrals**: Turn happy customers into advocates
3. **Content machine**: Stick to sustainable schedule
4. **Product development**: Build the #1 requested feature
5. **Scaling**: Can you get to 20 members? 30?

**What to stop doing**:

- Any content format that didn't perform
- Any platform that got zero engagement
- Anything that felt like grinding with no results

**What to start doing**:

- More of what worked
- Test new distribution channels (maybe podcasts, partnerships)
- Build the product more, market a bit less (you have customers now!)

---

**Next Steps**: Days 29-30 are for reflection, planning Month 2, and celebrating what you've built.
