# 🎯 Personalized Paths Strategy

> **Core Insight**: Users don't want to "learn a language." They want to achieve a specific outcome: talk to partner's mom, not embarrass themselves at dinner, stay in the loop when kids speak French. Personalized Paths deliver exactly that.

---

## What is a Personalized Path?

A custom 2-4 week scenario journey built specifically for ONE user's goal, based on a 15-minute discovery call.

**Example**: Chika wants to communicate with her French partner's mom and stay in the loop when they have kids.

Her Personalized Path:
```
Week 1: Basic mother-in-law conversations
  - Day 1: First meeting greetings
  - Day 2: Complimenting her cooking
  - Day 3: Talking about your work
  - Day 4: Asking about her son's childhood
  - Day 5: Phone call basics
  - Day 6: Saying goodbye warmly
  - Day 7: Review scenario (full dinner simulation)

Week 2: Deeper family conversations
  - Day 8: Discussing future plans
  - Day 9: Asking for advice (builds trust)
  - Day 10: Sharing good news
  - Day 11: Navigating disagreement gently
  - Day 12: Holiday/celebration vocabulary
  - Day 13: Talking about children/family
  - Day 14: Final simulation (extended family dinner)
```

---

## Pricing Structure

### For Founding Members (Current WhatsApp Group)

| Offer | Price | What's Included |
|-------|-------|-----------------|
| Personalized Path | **FREE** | 15-min call + 14-day custom journey |
| Base subscription | $15/month | What they already have |

**Rationale**: They're beta testers. Their feedback and testimonials are worth more than $49. Treat them like gold.

### For New Users

| Tier | Price | What's Included |
|------|-------|-----------------|
| **2-Week Path** | $49 | 15-min call + 14 custom scenarios + async check-ins |
| **4-Week Path** | $79 | 15-min call + 28 custom scenarios + weekly check-ins |
| **Monthly Custom** | $29/month add-on | 4 new custom scenarios/month + priority support |

### Subscription Tiers (Updated)

| Tier | Monthly | Includes |
|------|---------|----------|
| **Free** | $0 | 3 conversations/month |
| **Plus** | $14/month | Unlimited conversations |
| **Pro** | $29/month | Unlimited + 4 custom scenarios/month |
| **Path** | $49 one-time | 2-week personalized journey (includes 1 month Plus) |

---

## The 15-Minute Discovery Call

### What to Ask

1. **The Goal** (2 min)
   - "What specific situation are you preparing for?"
   - "When is this happening?" (creates urgency)

2. **The People** (2 min)
   - "Who will you be talking to?"
   - "What's their personality like?"
   - "What do they care about?"

3. **The Fear** (2 min)
   - "What are you most worried about?"
   - "What would be embarrassing?"
   - "What would success look like?"

4. **Current Level** (2 min)
   - "How much can you understand vs speak?"
   - "What topics are you comfortable with?"
   - "Where do you freeze up?"

5. **Specific Situations** (5 min)
   - "Walk me through a typical interaction"
   - "What questions might they ask you?"
   - "What do you want to be able to say?"

6. **Wrap-up** (2 min)
   - "Anything else I should know?"
   - "I'll have your first scenario ready by [date]"

### Call Template (Copy/Paste for Notes)

```
User:
Language:
Goal:
Timeline:
Who they're talking to:
Biggest fear:
Current level:
Specific situations needed:
Notes:
```

---

## Scalability Roadmap

### Phase 1: Full Manual (Now)
- **Process**: 15-min call → manually create 14 scenarios
- **Time per user**: ~1 hour
- **Capacity**: 5-10 users/week
- **Price**: Free (founding) / $49 (new)
- **Goal**: Learn patterns, build templates

### Phase 2: Templated (After 20 users)
- **Process**: Call → fill in template variables
- **Time per user**: 30-40 min
- **Capacity**: 10-15 users/week
- **Price**: $49-79
- **Goal**: Create 5-6 reusable path templates

### Phase 3: AI-Assisted (After 50 users)
- **Process**: Intake form → AI drafts → human review
- **Time per user**: 15-20 min
- **Capacity**: 20-30 users/week
- **Price**: $49-79
- **Goal**: Reduce manual work by 70%

### Phase 4: Self-Serve (After 100+ users)
- **Process**: User selects path → customizes via form → auto-generated
- **Time per user**: 0 (automated)
- **Capacity**: Unlimited
- **Price**: $19-29/month tier
- **Goal**: Productized paths

---

## Path Templates (Build as You Go)

After doing 20+ manual paths, you'll see patterns. Document them here:

### Template: "Meet Partner's Family"
**Target**: Sofia persona (bilingual spouse)
**Duration**: 2 weeks
**Scenarios**:
- Week 1: First impressions, basic politeness, handling questions
- Week 2: Deeper conversations, awkward moments, repair phrases

### Template: "Heritage Speaker Recovery"
**Target**: David persona (reconnecting with roots)
**Duration**: 2-4 weeks
**Scenarios**:
- Week 1: Overcoming shame, basic reconnection
- Week 2: Family stories, emotional vocabulary
- Week 3-4: Deeper family relationships, cultural topics

### Template: "Expat Life Prep"
**Target**: Jamie/Akemi persona (moving abroad)
**Duration**: 4 weeks
**Scenarios**:
- Week 1: Daily survival (shopping, transit, services)
- Week 2: Social situations (neighbors, casual friends)
- Week 3: Professional contexts (office, meetings)
- Week 4: Deeper integration (friendships, local culture)

### Template: "Fluency Maintenance"
**Target**: Scott persona (busy, maintaining skills)
**Duration**: Ongoing
**Format**: 5-min daily scenarios with variety
**Scenarios**: Rotating topics to prevent repetition

---

## Metrics to Track

### Per Path
- Completion rate (how many finish all 14/28 days?)
- Scenarios completed per day
- User-reported confidence change (before/after)
- Conversion to subscription after path ends

### Overall
- Paths sold per week
- Revenue from paths vs subscriptions
- Time spent per path (track for efficiency)
- Templates created (track for scalability)
- NPS from path users vs regular users

---

## Founding Member Path Tracker

| User | Language | Goal | Call Date | Path Started | Status |
|------|----------|------|-----------|--------------|--------|
| Robert | Japanese | Formal/stranger confidence | | | |
| Chika | French | Partner's mom + future kids | | | |
| Bri | Japanese | Work/live in Japan | | | |
| Akemi | Japanese | Part-time Japan residence | | | |
| Scott | Spanish | Fluency maintenance | | | |

---

## Sample Outreach Messages

### For Founding Members (Free Offer)

```
Hey [Name]! Quick update - I'm testing a new "Personalized Path" feature
where I build you custom scenarios based on your specific goals.

For founding members like you, I want to include this at no extra charge.
You're helping me build this thing and your feedback is worth way more than $15.

Want me to set up a quick 15-min call to create your custom 2-week path?
Just pick a time: [calendar link]
```

### For New Users ($49 Offer)

```
Hey [Name]! I saw your post about [specific situation].

I run Kaiwa, a conversation practice app, and I'm offering something
that might help: a Personalized Path.

Here's how it works:
- 15-min call where you tell me exactly what situations you need
- I build you 14 custom scenarios (one per day for 2 weeks)
- Each scenario is tailored to YOUR specific goal

It's $49 for the full 2-week path. Interested?
```

---

## Integration with Existing Strategy

### How Paths Fit the Funnel

```
HIGH-INTENT TRAFFIC (Reddit/Quora)
         ↓
LEAD MAGNET or helpful comment
         ↓
FREE TRIAL (3 conversations)
         ↓
    ┌────┴────┐
    ↓         ↓
$14/mo    $49 Path
 Plus    (one-time)
    ↓         ↓
    └────┬────┘
         ↓
$29/mo Pro (ongoing custom)
```

### Paths as Conversion Tool

- User tries free tier → likes it but needs more structure → Path
- User sees Reddit comment → wants personalized help → Path
- User churns from Plus → win-back with Path offer

---

## FAQ

**Q: What if someone wants a language I don't speak?**
A: You don't need to speak it. You're designing the *scenarios* and *progression*, not teaching grammar. AI handles the actual conversation.

**Q: What if someone doesn't complete the path?**
A: Check in at Day 3 and Day 7. If they're struggling, offer to adjust. Completion rate is a key metric.

**Q: How do I create scenarios quickly?**
A: Build templates. After 10 "Meet the Parents" paths, you'll have a template. Just swap names and specific situations.

**Q: Should I offer refunds?**
A: Yes, for founding members (goodwill). For paid users, offer to rebuild the path if it's not working, not a refund.

---

**Last Updated**: November 2025
**Status**: Phase 1 (Manual)
**Next Milestone**: Complete 10 paths → Create first template
