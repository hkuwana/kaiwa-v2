# Kaiwa Founding Members Launch Plan

**Complete Strategy & Execution Guide**

**Created:** November 22, 2024
**Launch Deadline:** Tomorrow Evening
**Goal:** Fill final 2 Founding Member spots (10/12 currently filled)

---

## 📋 Table of Contents

1. [Program Overview](#program-overview)
2. [Pricing & Value Proposition](#pricing--value-proposition)
3. [The 3 Initial Actions for Members](#the-3-initial-actions-for-members)
4. [Target Contacts & Messaging](#target-contacts--messaging)
5. [Marketing Strategy (Dual Approach)](#marketing-strategy-dual-approach)
6. [Payment & Operations Setup](#payment--operations-setup)
7. [Action Steps Timeline](#action-steps-timeline)
8. [Email & Message Templates](#email--message-templates)
9. [Success Metrics](#success-metrics)

---

## Program Overview

### What It's Called:

**"Kaiwa Founding Members"** (not "Product Advisory Board")

### Why the Name Change:

- "Advisory Board" + paying = feels like "pay to work"
- "Founding Members" + paying = feels like "early adopter discount"
- Better positioning: exclusive club, not unpaid labor

### Program Structure:

- **Total spots:** 12 people
- **Currently filled:** 10 spots
- **Remaining:** 2 spots
- **Deadline:** Tomorrow evening

---

## Pricing & Value Proposition

### Founding Member Pricing:

**$10/month (locked in forever)**

**Why $10/month:**

- Covers your API costs (OpenAI Realtime API: ~$10/user/month for typical use)
- You're not making profit on these 12 people
- It's an investment in getting the right early users
- Long-term "loss" worth it for feedback, testimonials, and loyalty

**Regular pricing when you launch publicly:**

- $19/month standard

**Value proposition:**

- 50% discount locked in forever
- Even if you raise prices to $29/month later, they stay at $10
- Founding members are rewarded for early support

### What's Included:

✅ $10/month rate (locked in forever)
✅ Unlimited access to Kaiwa
✅ Monthly 1-hour video calls with you + other founding members
✅ Direct input on features and roadmap
✅ Exclusive WhatsApp/Slack/Discord community
✅ Early access to all new features
✅ "I was here from day 1" status

### Fair Use:

- Typical use: 15-25 conversations/month
- If someone consistently uses 40+ conversations/month, you may ask them to discuss a custom plan
- Be transparent but fair

---

## The 3 Initial Actions for Members

When someone joins, they complete 3 tasks in their first week:

### ✅ Action 1: Fill Out Availability Poll (Due: 48 hours)

**Purpose:** Lock in first meeting date
**Tool:** Calendly or When2Meet
**Message:**

```
📅 First Task: When can you meet?

Please fill out this quick poll for our first Founding Members meeting:
[Calendly/When2Meet link]

We're looking at [next 2 weeks' date range] for a 1-hour video call.

This will be our kickoff where we:
- Get to know each other's language learning journeys
- Demo the current product
- Gather your initial reactions
- Decide what to prioritize

⏰ Please respond by [date 48 hours from now] so I can lock in a time!
```

### ✅ Action 2: Complete Your First Conversation (Due: 5 days)

**Purpose:** Get hands-on with product immediately
**Message:**

```
🗣️ Second Task: Try Kaiwa!

Please try at least one conversation scenario on Kaiwa this week.

**Your mission:** Complete a full conversation and review the analysis/feedback.

**What to note:**
- What felt natural vs confusing?
- Did anything surprise you (good or bad)?
- What's missing that you wish was there?

You can share feedback in our group or bring it to our first meeting.

**Access Link:** [link]
**Login credentials:** [credentials]
```

### ✅ Action 3: Share Your Story (Due: Before first meeting)

**Purpose:** Build community and gather persona data
**Message:**

```
💭 Third Task: Tell us your story

In our Founding Members group [WhatsApp/Slack/Discord link], please introduce yourself:

1. What language are you learning?
2. What's the ONE conversation you're preparing for?
3. Why does this conversation matter to you?

This helps everyone get to know each other and helps me understand what scenarios to prioritize building.

No essay needed - 2-3 sentences is perfect!
```

---

## Target Contacts & Messaging

### Priority 1: Maxi (WhatsApp) - SEND TONIGHT

**Context:** Met at Jason Calacanis event, wife is learning language to meet partner's family (perfect ICP)

**Message:**

```
Hey Maxi!

I'm locking in 12 Founding Members for Kaiwa this week

Your wife came to mind immediately

Founding Members pay $10/month (covers my API costs) vs $19/month regular price

They get locked in at $10 forever + monthly calls with me + help shape what we build

2 spots left, closing tomorrow evening

Based on what you mentioned about her, she'd be perfect

Want me to add her? Just need a yes 👍
```

---

### Priority 2: Brown Language Ambassadors (Email) - SEND TONIGHT

**Context:** They're peer language mentors at Brown, perfect for Founding Members or Student Ambassadors

**Contacts:**

1. Emma Brignall '27 - emma_brignall@brown.edu
2. Martina Herman '26 - martina_herman@brown.edu
3. Farah Akbari '28 - farahnaz_akbari@brown.edu
4. Mario Joyo Villanueva '28 - mario_joyo_villanueva@brown.edu

**Message:**

```
Subject: Kaiwa Founding Members - $10/month (vs $19 regular)

Hi [Name],

Hiro (Brown '17) here. I'm locking in 12 Founding Members for Kaiwa this week.

**The deal:**
$10/month founding member rate (locked in forever)
Regular users will pay $19/month

**As a Founding Member:**
✅ $10/month rate never changes
✅ Monthly calls with me + 11 other language learners
✅ Help shape what we build
✅ Early access to everything

**Why this price?**
$10/month covers my API costs. You're getting in at cost while I'm still building.

**The ask:**
Your honest feedback + monthly 1-hour calls

2 spots left. Closing tomorrow.

**Interested?** Reply "I'm in" and I'll send payment details.

Go Bruno,
Hiro

P.S. What languages are you working with as an Ambassador?
```

---

### Priority 3: Professor Shinohara (Email) - SEND TOMORROW

**Context:** Met for drinks, Waseda University professor researching L2 phonetic training

**Strategy:** Research collaboration (free for study participants) + Founding Members for other students

**Message:**

```
Subject: Great meeting you last week - quick collaboration idea

Hi Professor Shinohara,

Great having drinks last week! I've been thinking about your research on L2 phonetic training.

I'm building Kaiwa (trykaiwa.com) for high-stakes L2 conversation practice, and thought there might be a research collaboration opportunity.

**Research idea:**
Your expertise in L2 phonetics + usage data from Kaiwa could make for interesting research on scenario-based practice effects on production and anxiety reduction.

**Potential pilot:**
- 20-30 Waseda students (free access for study duration)
- Pre/post assessments (phonetic + anxiety measures)
- Co-authored publication

This could benefit your research, your students, and help validate Kaiwa academically.

**Would you be interested in a 15-min call to discuss?**

Also: If you know students who might benefit outside of a formal study, I'm offering 2 Founding Member spots ($10/month vs $19 regular price).

No pressure either way - just thought the alignment was worth exploring!

Best,
Hiro

P.S. Also happy to just grab coffee next time I'm in Tokyo and hear your thoughts on L2 practice tools!
```

---

### Priority 4: Your "Few Other People"

**Who are they?** List them here:

- [Add names and context]

**Template to customize:**

```
Hey [Name]!

Quick reach-out: I'm closing invites for Kaiwa Founding Members today (literally have 2 spots left).

I thought of you because [specific reason: "you mentioned learning Japanese" / "you're preparing for X" / etc].

Founding Members get:
✅ $10/month rate forever (vs $19 regular)
✅ Monthly calls with me + 10 others
✅ Help shape what we build
✅ Community of people in similar situations

Need to finalize by tomorrow evening.

Interested? Just reply "I'm in"!

No worries if not - would still love to catch up about [their situation] sometime!

[Your name]
```

---

## Marketing Strategy (Dual Approach)

### Approach A: Founding Members (High-Touch)

**Target:** 12 people total
**Goal:** Shape the product, deep feedback
**Time:** 1 hour/month meetings
**Compensation:** $10/month (vs $19 regular)
**Status:** 10/12 filled, need 2 more

### Approach B: Student Ambassador Program (Scalable)

**Target:** 2-3 students initially (scalable to 50+)
**Goal:** Campus distribution, word of mouth
**Time:** 2-3 hours/month activities
**Compensation:** $50/month stipend + free access

**How they work together:**

```
Student Ambassadors (awareness)
    ↓
Drive signups and trials
    ↓
Best users → invited to Founding Members
    ↓
Founding Members → deep product feedback
    ↓
Product improves
    ↓
Ambassadors have better product to promote
    ↓
Cycle repeats
```

---

### Student Ambassador Program (Future Phase)

**When to launch:** After filling Founding Members spots (Week 2)

**Value prop for students:**

- $50/month stipend
- Free lifetime access to Kaiwa
- "Ambassador" credential (resume builder)
- Direct line to founder
- Exclusive community with other ambassadors

**What they do:**

- Share Kaiwa with 5-10 students per semester
- Post on social media 2-3x/month
- Host 1 info session or demo per semester
- Collect feedback from users
- Report monthly

**Student Ambassador Pitch (for Brown Language Ambassadors who decline Founding Members):**

```
Subject: Different opportunity - Student Ambassador ($50/month)

Hi [Name],

Even if Founding Members isn't your thing, I have another idea:

**Kaiwa Student Ambassador Program**

What you get:
✅ $50/month stipend
✅ Free lifetime access
✅ "Founding Ambassador" credential
✅ Direct access to me (fellow Brown alum)

What you do:
📱 Post 2-3x/month on social (we give you templates)
🎤 Host 1 info session per semester (we give you materials)
💬 Share with students you work with

Time: 2-3 hours/month

This is literally what you already do as a Language Ambassador, but you'd get paid!

Only taking 2-3 Founding Ambassadors at Brown.

Interested? Reply "tell me more"!

Go Bruno,
Hiro
```

---

### Why NOT to Do Broad Student Marketing (Yet)

**Cost Reality:**

- Heavy student user: $10-15/month in API costs
- Students willing to pay: $0-10/month
- 50 free students = $500-750/month burn

**Better strategy:**

- Focus on paying customers (professionals with urgency)
- Do ONE strategic student partnership (Shinohara research = controlled cost)
- Launch Student Ambassador program (2-3 paid ambassadors, not 100 free users)
- Add student discount tier later ($9/month) but don't aggressively market to students yet

**When to revisit:**

- After you have 50+ paying customers
- After you've validated product-market fit
- After you have more budget for student acquisition

---

## Payment & Operations Setup

### Payment Collection (Choose One):

**Option 1: Stripe Subscription (RECOMMENDED)**

```
Steps:
1. Log into Stripe dashboard
2. Create Product: "Kaiwa Founding Members"
3. Set price: $10/month recurring
4. Generate payment link
5. Send link to members after they say "I'm in"

Pros: Professional, automatic billing, easy to manage
Time: 10 minutes to set up
```

**Option 2: Manual (Venmo/PayPal)**

```
Steps:
1. Request $10/month from each member manually
2. Track in spreadsheet
3. Follow up on late payments

Pros: Simple, no Stripe fees
Cons: Not scalable, unprofessional, manual work
```

**Recommendation:** Use Stripe. It's worth the 2.9% fee for automatic billing.

---

### Community Platform (Choose One):

**Option 1: WhatsApp Group**

- Pros: Everyone has it, low friction, instant
- Cons: Less organized, hard to search, casual

**Option 2: Slack Workspace**

- Pros: Professional, organized, searchable
- Cons: Another app to check, some learning curve

**Option 3: Discord Server**

- Pros: Good for communities, voice channels, organized
- Cons: Less professional, younger demographic

**Recommendation:** WhatsApp for simplicity, or Slack if you want more professional feel.

---

### WhatsApp Group Welcome Message:

```
Welcome to Kaiwa Founding Members! 🗣️

You're one of 12 people who will help build Kaiwa from the ground up.

**What happens here:**
✅ Share your Kaiwa experiences
✅ Give feedback on features
✅ Connect with other language learners
✅ Quick polls and questions from me
✅ Updates on product development

**Ground rules:**
- Be supportive (we're all learning!)
- Share wins and challenges
- Candid feedback is valuable
- Help each other

**First prompt: Introduce yourself!**
- What language are you learning?
- What conversation are you preparing for?
- Where are you based?

Let's build something great together!

Hiro
Founder, Kaiwa
```

---

## Action Steps Timeline

### TONIGHT (Next 60 minutes)

**Step 1: Send WhatsApp to Maxi** (5 min)

- [ ] Copy message from "Priority 1" section
- [ ] Send to Maxi on WhatsApp
- [ ] Set reminder to follow up tomorrow if no response

**Step 2: Email Brown Language Ambassadors** (20 min)

- [ ] Copy email from "Priority 2" section
- [ ] Send individually to all 4:
  - [ ] Emma Brignall (emma_brignall@brown.edu)
  - [ ] Martina Herman (martina_herman@brown.edu)
  - [ ] Farah Akbari (farahnaz_akbari@brown.edu)
  - [ ] Mario Joyo Villanueva (mario_joyo_villanueva@brown.edu)

**Step 3: Email Your Other Contacts** (15 min)

- [ ] List your "few other people" who showed interest
- [ ] Customize template from "Priority 4" section
- [ ] Send individually

**Step 4: Set Up Payment** (10 min)

- [ ] Create Stripe product: "Kaiwa Founding Members - $10/month"
- [ ] Generate payment link
- [ ] Test it yourself
- [ ] Save link for welcome emails

**Step 5: Create Community Group** (10 min)

- [ ] Create WhatsApp/Slack/Discord group
- [ ] Name it "Kaiwa Founding Members"
- [ ] Write welcome message (use template above)
- [ ] DON'T invite anyone yet (wait for confirmations)

**Step 6: Set Deadline Reminder** (1 min)

- [ ] Phone reminder for tomorrow evening
- [ ] Email reminder for tomorrow evening
- [ ] "Close Founding Members roster"

---

### TOMORROW MORNING

**Step 7: Check Responses** (10 min)

- [ ] Who replied to Maxi message?
- [ ] Who replied from Brown students?
- [ ] Who replied from other contacts?
- [ ] Tally: How many "yes" responses?

**Step 8: Send to Professor Shinohara** (5 min)

- [ ] Copy email from "Priority 3" section
- [ ] Send to y.shinohara@waseda.jp
- [ ] No urgency on this one (different track)

**Step 9: Follow Up on Non-Responses** (10 min)

- [ ] If Maxi didn't respond, send gentle nudge
- [ ] If Brown students didn't respond, send follow-up
- [ ] Give them until evening deadline

---

### TOMORROW EVENING (Deadline)

**Step 10: Close Roster** (30 min)

- [ ] Accept first 2 "yes" responses (or best 2 if multiple)
- [ ] Send welcome emails to new members
- [ ] Send payment links
- [ ] Invite to community group

**Step 11: Send Decline Emails** (15 min)

- [ ] To people who said "no" - thank them
- [ ] To people who didn't respond - close the opportunity
- [ ] Keep door open for future opportunities

**Step 12: Update Tracking** (10 min)

- [ ] Mark roster as "12/12 FILLED"
- [ ] Create member spreadsheet (see template below)
- [ ] Schedule first meeting using When2Meet/Calendly

---

### WEEK 2

**Step 13: Onboard New Members** (Ongoing)

- [ ] Monitor payment confirmations
- [ ] Send 3 initial tasks to each member
- [ ] Answer questions in community group
- [ ] Lock in first meeting time

**Step 14: First Meeting** (90 min)

- [ ] Host first Founding Members call
- [ ] Introductions (each person shares story)
- [ ] Product demo
- [ ] Open feedback session
- [ ] Schedule next meeting

**Step 15: Process Feedback** (30 min)

- [ ] Write up meeting notes
- [ ] Share notes with group
- [ ] Identify top 3 feature requests
- [ ] Thank everyone for attending

---

## Email & Message Templates

### Welcome Email (After They Say "I'm In")

```
Subject: Welcome to Kaiwa Founding Members!

Hi [Name],

Welcome to Kaiwa Founding Members! 🎉

You're one of 12 people who will help build Kaiwa from the ground up.

**Next steps:**

1️⃣ **Payment setup** (2 min)
👉 [Stripe payment link]
$10/month founding member rate (locked in forever)

2️⃣ **Join our group** (1 min)
👉 [WhatsApp/Slack/Discord invite]
This is where we'll coordinate between monthly calls

3️⃣ **Access Kaiwa** (5 min)
👉 [Login link]
Try at least one conversation this week

4️⃣ **First meeting poll** (2 min)
👉 [Calendly/When2Meet link]
We'll schedule our first call for next week

**What to expect:**
- First meeting: Week of [date]
- Monthly calls: 1 hour, casual discussion
- WhatsApp group: Async feedback and updates
- Direct line to me: Questions? Just ask!

Thanks for being here from day one. Let's build something great together.

Hiro
Founder, Kaiwa

P.S. If you have any issues with payment or access, just reply to this email!
```

---

### Follow-Up Message (If No Response After 8 Hours)

**For Maxi (WhatsApp):**

```
Bumping this - need to finalize by tonight!

I know your wife would be perfect for this. Even just a quick "yes" or "not right now" helps me plan.

Thanks!
```

**For Brown Students (Email):**

```
Subject: Re: Kaiwa Founding Members - closing tonight

Hi [Name],

Just bumping this in case it got buried!

Closing roster tonight - 2 spots left.

If you're interested, just reply "I'm in"!

If not, no worries - appreciate you considering it.

Go Bruno,
Hiro
```

---

### Decline Email (If They Say No)

```
Subject: Re: Kaiwa Founding Members

Hi [Name],

No worries at all - thanks for letting me know!

I'll keep you on the early access list and reach out when we officially launch.

In the meantime, if you have any feedback on language learning tools or know anyone who might be a good fit, just reply - I'm always around!

Best,
Hiro
```

---

### Opportunity Closed Email (If They Don't Respond by Deadline)

```
Subject: Re: Kaiwa Founding Members - roster filled

Hi [Name],

Just wanted to let you know I've filled all 12 Founding Member spots.

If your situation changes or you're interested in trying Kaiwa when I launch publicly (at $19/month), just let me know!

Thanks for considering it.

Best,
Hiro

P.S. If you know anyone preparing to meet their partner's family in another language, reconnecting with heritage language, or relocating abroad - send them my way!
```

---

## Success Metrics

### Week 1 Targets (By Tomorrow Evening):

- [ ] 12/12 Founding Member spots filled
- [ ] All 12 members paid (Stripe subscriptions active)
- [ ] All 12 members in community group
- [ ] First meeting scheduled (>80% confirmed availability)

### Week 2 Targets:

- [ ] First Founding Members meeting completed (>80% attendance)
- [ ] Top 3 feature requests identified from feedback
- [ ] All 12 members completed at least 1 conversation
- [ ] 10+ intro messages in community group

### Month 1 Targets:

- [ ] 2-3 monthly meetings completed
- [ ] Average 10+ conversations per member
- [ ] 80%+ retention (still paying)
- [ ] 2-3 testimonial quotes collected
- [ ] Clear product priorities from member feedback

---

## Member Tracking Spreadsheet

Create a Google Sheet with these columns:

| Name  | Email             | Source | Payment Status | Conversations This Month | Last Active | Attending Meetings | Notes                       |
| ----- | ----------------- | ------ | -------------- | ------------------------ | ----------- | ------------------ | --------------------------- |
| Sarah | sarah@example.com | USJC   | ✅ Active      | 12                       | 2024-11-21  | 2/2                | Great feedback on scenarios |
| David | david@example.com | Friend | ✅ Active      | 8                        | 2024-11-20  | 1/2                | Requested Spanish scenarios |

**Track monthly:**

- Payment status (active, past due, cancelled)
- Usage (conversations per month)
- Engagement (attending meetings, posting in group)
- Feedback quality (are they giving useful insights?)

---

## Key Principles to Remember

### 1. Founding Members = Investment, Not Revenue

These 12 people at $10/month are:

- ✅ Worth it for feedback and testimonials
- ✅ Worth it for loyalty and referrals
- ✅ Worth it even if you "lose money" long-term
- ✅ Building the product WITH the right people

### 2. Keep It Simple

- Don't overcomplicate the offer
- Don't add tiers or options
- Just: $10/month, 12 spots, locked in forever

### 3. Be Transparent

- "This covers my API costs"
- "I'm not making profit on you 12"
- "Later users pay $19-29/month"
- Honesty builds trust

### 4. Execute Fast

- You have momentum NOW
- 13-14 people already interested
- Send messages tonight
- Close roster tomorrow
- Launch Week 2

### 5. Students = Later

- Don't chase free student users yet
- Do ONE strategic partnership (Shinohara research)
- Focus on paying customers first
- Add student tier when you have budget

---

## Quick Reference: Who Gets What

### Founding Members (12 people):

- $10/month (locked in forever)
- Unlimited conversations
- Monthly calls
- Community group
- Early access

### Student Ambassadors (2-3 people, launch Week 2):

- $50/month stipend
- Free access
- Promote to peers
- 2-3 hours/month

### Research Participants (20-30 people, Shinohara study):

- Free for study duration (1 semester)
- Pre/post assessments
- Academic collaboration

### Regular Users (launch later):

- $19/month standard
- No meetings
- Standard support

---

## Final Checklist Before Sending

- [ ] All messages reviewed and customized
- [ ] Stripe payment link created and tested
- [ ] Community group created (not yet populated)
- [ ] Calendar/scheduling tool ready
- [ ] Login credentials ready for new members
- [ ] Deadline set (tomorrow evening)
- [ ] Tracking spreadsheet created
- [ ] Welcome email template ready

---

## Emergency Contacts If Things Go Wrong

**If Stripe fails:**

- Use Venmo/PayPal temporarily, fix Stripe later

**If no one responds:**

- Send to more people from your existing list
- Extend deadline 24 hours
- Reach out personally (call/text)

**If too many people say yes:**

- Great problem! Accept first 2 or best 2
- Create waitlist for others
- Offer them "next cohort" in 3 months

**If confused about API costs:**

- Check OpenAI dashboard for actual usage
- Adjust fair use policy if needed
- Can always raise to $12-15/month for new members later

---

## You're Ready. Send the Messages Tonight.

Everything is prepared:
✅ Clear positioning (Founding Members, not Advisory Board)
✅ Fair pricing ($10/month covers costs)
✅ Strong value prop (locked-in rate + community)
✅ Ready templates (copy-paste and send)
✅ Clear timeline (tonight → tomorrow → Week 2)

**Stop strategizing. Start executing.**

**Send to:**

1. Maxi (WhatsApp) - 5 min
2. Brown students (Email) - 20 min
3. Other contacts (Email) - 15 min

**Tomorrow evening you'll have:**

- 12/12 Founding Members filled
- Revenue coming in ($120/month)
- Momentum building
- Real customers helping you build

**Let's go. 🚀**

---

**Last Updated:** November 22, 2024
**Next Review:** After filling all 12 spots
**Document Owner:** Hiro Kuwana, Founder
