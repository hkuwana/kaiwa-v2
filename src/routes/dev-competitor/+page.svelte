<script lang="ts">
  ...
  const timelineComparison = [
    {
      period: "Day 1",
      kaiwa: { title: "Start Speaking", desc: "Click and start a real conversation immediately", icon: "⚡" },
      others: { title: "Complete Tutorials", desc: "Watch intro videos, complete setup, unlock speaking", icon: "📚" }
    },
    {
      period: "Week 1", 
      kaiwa: { title: "20+ Real Conversations", desc: "Practice apologies, compliments, asking for help", icon: "💬" },
      others: { title: "Vocabulary Drills", desc: "Repeat words, match pictures, fill in blanks", icon: "🔄" }
    },
    {
      period: "Month 1",
      kaiwa: { title: "Unlimited Practice", desc: "Speak as much as you want, always free", icon: "♾️" },
      others: { title: "Hit Limits", desc: "Daily caps reached, premium upgrade required", icon: "🚫" }
    },
    {
      period: "Month 3",
      kaiwa: { title: "Personal Context", desc: "AI remembers your interests and builds on them", icon: "🌱" },
      others: { title: "Repetitive Topics", desc: "Same basic scenarios, no personal connection", icon: "🔁" }
    }
  ];
  ...
  const competitorProblems = [
    {
      ...
      detail: "Free users get limited daily voice usage. Paid users get more, but it's still capped.",
  ...
  const userTypes = {
    beginner: {
      ...
      comparisons: [
        { feature: "Getting started", kaiwa: "Click → Speak", others: "Lessons → Tests → Unlock", rating: 5 },
    ...
    intermediate: {
      ...
      comparisons: [
        { feature: "Conversation depth", kaiwa: "Complex emotions", others: "Basic topics", rating: 5 },
        { feature: "Personal relevance", kaiwa: "Remembers your life", others: "Generic scenarios", rating: 5 },
        { feature: "Cultural context", kaiwa: "Local phrases", others: "Textbook language", rating: 4 }
      ]
    },
    asian: {
      title: "Asian Language Learner",
      comparisons: [
        { feature: "Character support", kaiwa: "Furigana/romanization", others: "Raw characters", rating: 5 },
        { feature: "Reading anxiety", kaiwa: "Supported", others: "Blocked by scripts", rating: 5 },
        { feature: "Speaking confidence", kaiwa: "Can focus on speaking", others: "Stuck on reading", rating: 4 }
      ]
    }
  };
  ...
  function getPersonalizedResults() {
    const painPoints = answers.map(answerId => 
      quizQuestions.find(q => q.options.some(opt => opt.id === answerId))
...
<div class="container mx-auto max-w-6xl p-4">
  <div class="mb-8 text-center">
    ...
    <p class="mt-2 opacity-70">Choose a concept to explore different ways of showcasing Kaiwa's advantages</p>
  </div>

  <!-- Concept Selector -->
  <div class="mb-8">
    <div class="tabs tabs-boxed justify-center">
      <button class="tab {selectedConcept === 'quiz' ? 'tab-active' : ''}" 
              on:click={() => selectedConcept = 'quiz'}>
        🎯 Pain Point Quiz
      </button>
      <button class="tab {selectedConcept === 'timeline' ? 'tab-active' : ''}" 
              on:click={() => selectedConcept = 'timeline'}>
        ⏱️ Learning Journey
      </button>
      <button class="tab {selectedConcept === 'carousel' ? 'tab-active' : ''}" 
              on:click={() => selectedConcept = 'carousel'}>
        🎠 Problem Carousel
      </button>
      <button class="tab {selectedConcept === 'gallery' ? 'tab-active' : ''}" 
              on:click={() => selectedConcept = 'gallery'}>
        🖼️ Hover Gallery
      </button>
      <button class="tab {selectedConcept === 'tabs' ? 'tab-active' : ''}" 
              on:click={() => selectedConcept = 'tabs'}>
        📊 User Type Tabs
      </button>
    </div>
  </div>
  ...
  {#if selectedConcept === 'quiz'}
    <div class="space-y-6">
      <div class="text-center">
        ...
        <p class="mt-2 opacity-70">Discover which Kaiwa features solve your specific frustrations</p>
      </div>
        ...
        <div class="card bg-base-100 shadow-xl max-w-2xl mx-auto">
          <div class="card-body">
            <div class="flex justify-between items-center mb-4">
              ...
              <div class="text-sm opacity-70">{Math.round(((currentQuestion) / quizQuestions.length) * 100)}% complete</div>
            </div>
            
            <progress class="progress progress-primary w-full mb-4" 
                     value={currentQuestion} max={quizQuestions.length}></progress>
            
            <h4 class="text-lg font-medium mb-6">{quizQuestions[currentQuestion].question}</h4>
      ...
      {:else}
        <div class="card bg-base-100 shadow-xl max-w-2xl mx-auto">
          <div class="card-body text-center">
            <h3 class="card-title justify-center text-2xl mb-4">Your Personalized Results</h3>
            <div class="space-y-4">
                ...
                <div class="card bg-primary/10 border border-primary/20">
                  <div class="card-body py-4">
                    {#if painPoint === 'setup'}
                      ...
                      <p class="text-sm">No tutorials or setup required. Click once and start your first real conversation.</p>
                    {:else if painPoint === 'limits'}
                      <h4 class="font-bold">✅ Unlimited Free Speaking</h4>
                      <p class="text-sm">Speak as much as you want, whenever you want. No daily limits or paywalls.</p>
                    {:else if painPoint === 'relevance'}
                      <h4 class="font-bold">✅ Personally Relevant Conversations</h4>
                      <p class="text-sm">AI remembers your interests and creates conversations about topics you actually care about.</p>
                    {:else if painPoint === 'scripts'}
                      <h4 class="font-bold">✅ Reading Support for Asian Languages</h4>
                      <p class="text-sm">Furigana and romanization help you focus on speaking without getting stuck on characters.</p>
                    {:else if painPoint === 'emotion'}
                      <h4 class="font-bold">✅ Real Human Emotions</h4>
                      <p class="text-sm">Practice apologies, expressions of affection, conflict resolution—conversations that actually matter.</p>
                    {:else if painPoint === 'memory'}
                      <h4 class="font-bold">✅ Conversation Memory</h4>
                      <p class="text-sm">Each conversation builds on previous ones, creating continuity and deeper relationships.</p>
                    {:else if painPoint === 'culture'}
                      <h4 class="font-bold">✅ Cultural Insider Knowledge</h4>
                      <p class="text-sm">Learn phrases locals actually use and understand the cultural context behind them.</p>
                    {:else if painPoint === 'flow'}
                      <h4 class="font-bold">✅ Natural Conversation Flow</h4>
                      <p class="text-sm">True back-and-forth dialogue that feels human, with natural turn-taking and responses.</p>
                    {/if}
                  </div>
                </div>
              {/each}
            ...
            <div class="card-actions justify-center mt-6">
              ...
              <button class="btn btn-ghost" on:click={resetQuiz}>Retake Quiz</button>
            </div>
          </div>
        </div>
      {/if}
    ...
    <div class="space-y-6">
      ...
      <div class="grid gap-8">
        {#each timelineComparison as period}
          <div class="diff aspect-[16/9] max-w-4xl mx-auto">
          ...
          <div class="timeline-box max-w-4xl mx-auto">
            <div class="grid md:grid-cols-3 gap-6 items-center">
              <!-- Timeline marker -->
              <div class="text-center">
                <div class="text-2xl font-bold text-primary">{period.period}</div>
                <div class="w-4 h-4 bg-primary rounded-full mx-auto mt-2"></div>
              </div>
              ...
              <div class="card bg-base-200 shadow">
                <div class="card-body">
                  ...
                  <div class="text-xs mt-2 opacity-60">Typical language learning apps</div>
                </div>
              </div>
              ...
              <div class="card bg-primary text-primary-content shadow-lg">
                <div class="card-body">
                  ...
                  <div class="text-xs mt-2 opacity-75">With Kaiwa</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="carousel w-full">
        {#each competitorProblems as problem, i}
          <div id="slide{i+1}" class="carousel-item relative w-full">
            <div class="card {problem.color} shadow-xl mx-auto max-w-2xl w-full">
              <div class="card-body">
            ...
            <div class="absolute flex justify-between transform -translate-y-1/2 left-5 right-5 top-1/2">
              ...
              <a href="#slide{i === competitorProblems.length - 1 ? 1 : i + 2}" class="btn btn-circle">❯</a>
            </div>
          </div>
        {/each}
      </div>
        ...
        {#each scenarioGallery as scenario}
          <div class="card bg-base-100 shadow-xl hover:shadow-2xl transition-all duration-300 group">
            <div class="card-body">
              ...
              <p class="text-sm opacity-70 mb-4">{scenario.description}</p>
              
              <div class="space-y-3">
                <div class="mockup-browser bg-base-300 border group-hover:bg-success group-hover:text-success-content transition-colors">
                  <div class="mockup-browser-toolbar">
                    <div class="input">kaiwa.app</div>
                  </div>
                  <div class="flex justify-center px-4 py-16 bg-base-200 group-hover:bg-success-content">
                    <div class="text-center">
                ...
                <div class="mockup-browser bg-base-300 border group-hover:bg-error group-hover:text-error-content transition-colors">
                  <div class="mockup-browser-toolbar">
                    <div class="input">other-app.com</div>
                  </div>
                  <div class="flex justify-center px-4 py-16 bg-base-200 group-hover:bg-error-content">
                    <div class="text-center">
                      <div class="text-4xl mb-2">😕</div>
              ...
              <div class="tooltip tooltip-bottom mt-4" data-tip="Click to try this scenario">
                <button class="btn btn-primary btn-sm w-full">Experience This</button>
              </div>
            </div>
          </div>
        {/each}
  ...
  {#if selectedConcept === 'tabs'}
    <div class="space-y-6">
      ...
      <div class="tabs tabs-lifted tabs-lg justify-center">
        {#each Object.entries(userTypes) as [key, userType]}
          <input type="radio" name="user_tabs" role="tab" class="tab" 
                 aria-label={userType.title} checked={key === 'beginner'} />
          <div role="tabpanel" class="tab-content bg-base-100 border-base-300 rounded-box p-6">
            <h3 class="text-xl font-bold mb-4">{userType.title}</h3>
                ...
                <div class="card bg-base-50 border border-base-300">
                  <div class="card-body py-4">
                      ...
                      <div class="rating rating-sm">
                        {#each Array(5) as _, i}
                          <input type="radio" class="mask mask-star-2 bg-orange-400" 
                                 checked={i < comparison.rating} disabled />
                        {/each}
                      </div>
                    </div>
                    ...
                    <div class="diff aspect-[4/1]">
                      <div class="diff-item-1">
                        <div class="bg-base-300 text-base-content grid place-content-center text-sm font-medium p-2">
                          Others: {comparison.others}
                        </div>
                      </div>
                      <div class="diff-item-2">
                        <div class="bg-primary text-primary-content grid place-content-center text-sm font-medium p-2">
