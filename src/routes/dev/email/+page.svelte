<script lang="ts">
	...
	let showSuperhumanPreview = $state(false);
	let superhumanPreviewHtml = $state<string>('');
	...
	async function previewSuperhumanEmail() {
		isLoadingPreview = true;

		try {
			const response = await fetch('/api/admin/send-superhuman-update?preview=true');

			if (response.ok) {
				superhumanPreviewHtml = await response.text();
				showSuperhumanPreview = true;

				posthogManager.trackEvent('dev_superhuman_email_preview_viewed');
			} else {
...
<div class="min-h-screen bg-base-200 p-8">
	<div class="mx-auto max-w-6xl">
		...
		<!-- SUPERHUMAN EMAIL QUICK PREVIEW -->
			...
			<div class="card-body">
				<div class="mb-4 flex items-center justify-between">
					<div>
						...
						<p class="text-sm text-base-content/60">
							Edit your Superhuman-style weekly update and see it live
						</p>
					</div>
					<div class="badge badge-primary badge-lg">Superhuman Style</div>
				</div>
						...
						<ol class="mt-2 list-decimal list-inside space-y-1">
							<li>
								...
								<code class="bg-base-200 px-2 py-1 rounded text-xs"
									>src/lib/emails/campaigns/product-updates/superhuman-template.ts</code
								>
							</li>
							<li>Save the file</li>
				...
				<div class="flex gap-3">
					<button
						...
						onclick={previewSuperhumanEmail}
					...
					<a
						href="/api/admin/send-superhuman-update?preview=true"
						target="_blank"
		...
		<!-- Superhuman Preview Display -->
		{#if showSuperhumanPreview}
			<div class="card mb-8 bg-base-100 shadow-xl">
				<div class="card-body">
					<div class="mb-4 flex items-center justify-between">
						...
						<button
							...
							onclick={() => (showSuperhumanPreview = false)}
						>
							✕ Close
						</button>
					</div>
					<div class="overflow-hidden rounded-lg border border-base-300 bg-white">
						<!-- Safe: superhumanPreviewHtml is email preview HTML generated by our template -->
						<!-- svelte-ignore svelte_no_at_html_tags -->
						{@html superhumanPreviewHtml}
					</div>
					<div class="mt-4 alert alert-success">
						...
						<div>
							...
							<div class="text-sm">
								...
								<code class="bg-base-200 px-2 py-1 rounded ml-2"
									>POST /api/admin/send-superhuman-update</code
								>
							</div>
						</div>
					</div>
