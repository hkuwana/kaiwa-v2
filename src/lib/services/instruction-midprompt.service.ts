...
export const CONVERSATION_TEMPLATES = {
  first_time_user: {
    phases: ['greeting', 'topic_exploration', 'confidence_building', 'closing'] as ConversationPhase[],
    duration: 300, // 5 minutes
    description: 'First-time user onboarding flow'
  },

  struggling_user: {
    phases: ['error_recovery', 'confidence_building', 'practice_activity', 'closing'] as ConversationPhase[],  
...
export function detectConversationTrigger(
  ...
  if (userInput.includes('I don\'t know') || userInput.includes('difficult')) return 'language_barrier';
...
export function getNextPhase(
  ...
  const standardFlow: ConversationPhase[] = [
    'greeting', 'topic_exploration', 'practice_activity', 'assessment', 'closing'
  ];
