...
export class AnalysisFindingsRepository {
	...
	async getFindingById(id: string): Promise<AnalysisFinding | null> {
		const result = await db.select().from(analysisFindings).where(eq(analysisFindings.id, id)).limit(1);
	...
	async listRecentFindings(options: { userId: string; limit?: number; languageId?: string }): Promise<AnalysisFinding[]> {
