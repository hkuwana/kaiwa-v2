	...
	>
		...
		>
			...
			<div class="flex items-center justify-between border-b border-base-200 p-6 pb-4">
				<div class="flex items-center gap-3">
					<div class="flex h-10 w-10 items-center justify-center rounded-full bg-primary/10">
						...
						{:else}
							<svg class="h-5 w-5 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 01 2 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
							</svg>
						{/if}
					</div>
							...
							{#each customScenarios.slice(0, 2) as scenario}
								<div class="flex items-center justify-between rounded bg-base-100 p-2">
									<div>
										...
										<div class="text-xs text-base-content/60">{scenario.category} • {scenario.difficulty}</div>
									</div>
									<div class="badge badge-primary badge-sm">{scenario.suggestedDuration}min</div>
								</div>
							{/each}
				...
				{#if isGuestUser && analysisResult?.analysisType === 'onboarding'}
					<div class="mb-4 rounded-lg bg-gradient-to-r from-primary/10 to-secondary/10 p-4">
						<div class="flex items-start gap-3">
							<svg class="mt-0.5 h-5 w-5 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
							</svg>
							<div>
								<div class="text-sm font-medium text-base-content">Create your free account to:</div>
			...
			<div class="flex gap-3 border-t border-base-200 p-6">
				<button class="btn btn-outline flex-1" onclick={onDismiss}>
					Maybe Later
				</button>
				
				{#if isGuestUser && analysisResult?.analysisType === 'onboarding'}
					<button class="btn btn-primary flex-1" onclick={onCreateAccount}>
						Create Free Account
					</button>
				{:else}
					<button class="btn btn-primary flex-1" onclick={onViewResults}>
						View Results
					</button>
				{/if}
			</div>
