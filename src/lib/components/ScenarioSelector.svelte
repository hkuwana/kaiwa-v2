...
<script lang="ts">
	...
	const { scenarios, selectedScenario, onScenarioSelect, disabled = false } = $props();
			...
			<div class="max-h-80 overflow-y-auto px-2">
				{#each scenarios as scenario (scenario.id)}
					<button
						...
						class="group btn my-1 flex w-full items-start justify-between rounded-xl px-4 py-3 text-left btn-ghost transition-colors duration-150 hover:bg-base-200/50"
					...
					>
						<div class="flex flex-col">
							...
							<span class="text-sm opacity-70">{scenario.description}</span>
						</div>
