...
<div class="confidence-tracker rounded-lg {colors.bg} {colors.border} border p-6">
		...
		{#if confidenceData.previousScore !== undefined}
			<div class="flex items-center gap-2">
				{#if confidenceChange > 0}
					...
					<span class="text-success font-medium">+{confidenceChange}</span>
				{:else if confidenceChange < 0}
					...
					<span class="text-warning font-medium">{confidenceChange}</span>
				{:else}
					<svg class="w-5 h-5 text-base-content/60" fill="currentColor" viewBox="0 0 20 20">
						<path fill-rule="evenodd" d="M3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd"></path>
					</svg>
					<span class="text-base-content/60 font-medium">Stable</span>
				{/if}
			</div>
		{/if}
	</div>
	...
	<!-- Progress bar -->
	<div class="mb-4">
		<div class="flex justify-between text-sm {colors.text} opacity-70 mb-1">
	...
	<!-- Toggle details -->
	<button
		class="btn btn-sm btn-ghost {colors.text} mb-4"
		onclick={() => showDetails = !showDetails}
	>
		{showDetails ? 'Hide' : 'Show'} Details
		<svg class="w-4 h-4 transition-transform" class:rotate-180={showDetails} fill="currentColor" viewBox="0 0 20 20">
			...
			<div>
				<h4 class="font-medium {colors.text} mb-2 flex items-center gap-2">
					<svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
						<path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-6-3a2 2 0 11-4 0 2 2 0 014 0zm-2 4a1 1 0 00-1 1v3a1 1 0 102 0v-3a1 1 0 00-1-1z" clip-rule="evenodd"></path>
					</svg>
			...
			<div>
				...
				<div class="grid gap-2">
					{#each confidenceData.milestones as milestone}
						<div class="flex items-center gap-3 p-2 bg-white/10 rounded">
