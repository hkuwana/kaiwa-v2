...
// Handles ONLY audio device management with event-driven architecture
...
export class AudioDeviceManager {
	...
	async getStream(deviceId?: string): Promise<MediaStream> {
		try {
			...
			// Emit success event
			eventBus.emit('audio.stream.ready', { stream: this.currentStream });

			return this.currentStream;
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : 'Unknown error';
			eventBus.emit('audio.stream.error', { error: errorMessage });
	...
	private setupAudioLevelMonitoring(stream: MediaStream): void {
		...
		this.levelInterval = window.setInterval(() => {
			...
			eventBus.emit('audio.level.update', {
				level,
				timestamp: Date.now()
			});
		}, 100);
	...
	private async detectDeviceChange(): Promise<void> {
		...
		if (!currentExists && devices.length > 0) {
			...
			eventBus.emit('audio.device.changed', {
