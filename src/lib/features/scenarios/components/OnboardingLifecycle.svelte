	...
	const steps = $derived(() => {
		const s = [
			{ id: 'select', label: 'Select', done: currentPath !== '/', active: currentPath === '/' },
			{
				id: 'practice',
				label: 'Practice',
				done: currentPath.startsWith('/analysis'),
				active: currentPath.startsWith('/conversation')
			},
	...
	<div class="flex items-center justify-center gap-2">
		{#each steps() as step, i (step.id)}
			<div class="flex items-center gap-2">
				<!-- Step Circle -->
				<div
					class="flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium {step.done
				...
				{#if i < steps().length - 1}
					<div class="h-0.5 w-6 {step.done ? 'bg-success' : 'bg-base-200'}"></div>
				{/if}
			</div>
		{/each}
		...
		<!-- Sign Up Link -->
		{#if currentPath.startsWith('/analysis')}
			<a href="/auth" class="ml-3 text-xs font-medium text-primary hover:underline"> Sign Up </a>
		{/if}
	</div>
{/if}
