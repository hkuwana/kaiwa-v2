<script lang="ts">
	...
	let selectedDuration = $derived(
		sessionDurations.find((d) => d.value === formData.sessionDuration)
	);
	let selectedPriority = $derived(
		skillsPriorities.find((p) => p.value === formData.skillsPriority)
	);
				...
				<div class="rounded-lg border-2 border-base-300 p-6">
					...
					<div class="space-y-4">
						...
						<div class="form-control">
							...
							<input type="date" class="input-bordered input" bind:value={formData.deadline} />
						</div>
					</div>
				</div>
				...
				<div class="rounded-lg border-2 border-base-300 p-6">
								...
								<label class="cursor-pointer">
									...
									>
										<div class="text-center">
											<div class="mb-2 text-2xl">{template.name.split(' ')[0]}</div>
					...
					<div>
						...
						<div class="grid grid-cols-2 gap-2 md:grid-cols-3">
							<label
								class="flex cursor-pointer items-center gap-2 rounded-lg border p-3 transition hover:bg-base-200"
							>
								<input
									type="checkbox"
							...
							{#each sessionDurations as duration}
								<label class="cursor-pointer">
									...
									<div
										...
										class:bg-primary={formData.sessionDuration === duration.value}
							...
							{#each feedbackStyles as style}
								<label class="cursor-pointer">
									...
									<div
										...
										class:bg-primary={formData.feedbackStyle === style.value}
							...
							{#each skillsPriorities as priority}
								<label class="cursor-pointer">
									...
									<div
										...
										class:bg-primary={formData.skillsPriority === priority.value}
				...
				<div class="rounded-lg border-2 border-success/30 bg-success/5 p-6">
					...
					<div class="space-y-2 text-sm">
						...
						<p>
							...
							{selectedDuration?.label} ({selectedFeedback?.label} feedback)
						</p>
